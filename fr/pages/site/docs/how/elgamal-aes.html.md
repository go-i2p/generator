...
Chaque message reçu a l'une de deux conditions possibles :

1. Il fait partie d'une session existante et contient une étiquette de
 session et un bloc chiffré avec AES
2. Il est pour une nouvelle session et contient à la fois des blocs
 chiffrés avec ElGamal et AES

Quand un routeur reçoit un message, il supposera d'abord qu'il provient
d'une session existante et tentera de chercher la balise de session,
puis déchiffrera les données suivantes en utilisant AES. Si cela échoue,
il supposera alors qu'il s'agit d'une nouvelle session et tentera de
déchiffrer le message en utilisant ElGamal.

## New Session Message Specification {#new}

Un message de nouvelle session ElGamal contient deux parties, un bloc
chiffré en ElGamal et un bloc chiffré en AES.
...
Les données ElGamal non chiffrées ont 222 octets de long, contenant :
...
more than the minimum required padding. Longueur
minimale : 48 octets
...
- La clé de nouvelle session est actuellement inutilisée et n'est
 jamais présente.
