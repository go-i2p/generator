...
- [Les attaques contre les ressources centralisées](#central)
- [Development attacks](#dev)
- [Implementation attacks](#impl)
- [Autres défenses](#blocklist)
...
quelqu'un de découvrir des informations que vous ne voulez pas qu'ils
sache ?\" - qui vous êtes, où vous êtes localisé, avec qui vous
communiquez, ou même quand vous communiquez. L'anonymat \"parfait\"
n'est pas un concept utile ici - le logiciel ne permettra pas de vous
rendre indiscernable des gens qui n'utilisent pas d'ordinateurs ou qui
ne sont pas sur Internet. Au lieu de cela, nous travaillons à fournir de
l'anonymat suffisant pour répondre aux besoins réels de quiconque nous
pouvons - de ceux naviguant simplement sur des sites Web, à ceux
échangeant des données, à ceux craintifs d'être découverts par
organisations puissantes ou états.
...
bon espoir à répondre à cette question en explorant comment I2P opère
sous diverses attaques afin que vous puissiez décider s'il répond à vos
besoins.

Nous accueillons des recherches plus approfondies et l'analyse de la
résistance d'I2P aux menaces décrites ci-dessous. Davantage d'examens et
de littérature existante (une grande partie de celle-ci est concentré
sur Tor) et de travaux originaux concentrés sur I2P sont nécessaires.

## Résumé de la topologie de réseau
...
- **I2P est un réseau de mélange à routes libres** - le créateur du
 message définit clairement la route d'envoi (le tunnel sortant) et
 le destinataire du message définit clairement la route de réception
 (le tunnel entrant).
- **I2P n'a pas de points d'entrée ni de sortie officiels** ; tous les
 pairs participent totalement au mélange et il n'y a pas de
 mandataire entrant ni de sortie pour la couche réseau (cependant, il
 existe quelques mandataires au niveau de la couche application)
- **I2P est entièrement distribué** - il n'y a aucun contrôle central
 ni autorité. On pourrait modifier quelques routeurs pour opérer des
 cascades de mélanges (construisant des tunnels et donnant les clés
 nécessaires pour contrôler le transfert au point de sortie de
 tunnel) ou des annuaires basés sur le profilage et la sélection,
 tout cela sans casser la compatibilité avec le reste du réseau, mais
 le faire n'est bien sûr pas nécessaire (et pourrait même nuire à
 l'anonymat de quelqu'un).
...
En théorie, les routeurs au long du chemin du message peuvent injecter
un nombre arbitraire d'étapes avant expédition du message au pair
suivant, quoique la mise en œuvre actuelle ne le fait pas.
...
Extrait des attaques et analyses mises de l'avant dans la [littérature
de l'anonymat](http://freehaven.net/anonbib/topic.html) (en grande
partie [Analyse du trafic : protocoles, attaques, problèmes de
conception et problèmes
ouverts](http://citeseer.ist.psu.edu/454354.html) \[site en anglais\]),
ce qui suit décrit brièvement une large gamme d'attaques ainsi que
plusieurs des défenses d'I2P. Nous mettons cette liste à jour pour
qu'elle comprennent les nouvelles attaques au fur et à mesure qu'elles
sont identifiées.

Cela inclue quelques attaques qui peuvent être uniques à I2P. Nous
n'avons pas de bonnes réponses pour toutes ces attaques, cependant nous
continuons à faire la recherche et améliorer nos défenses.
...
actuel. Bien que nous soyons conscients de certaines limites qui doivent
être examinées, I2P est conçu pour prendre en charge des centaines de
milliers voire des millions de participants. Alors que nous continuons à
passer le message et à étendre le réseau, ces attaques deviendront plus
difficiles.
...
### Les attaques contre des ressources centralisées {#central}
...
des mises à jour automatiques, le réseau est vulnérable à une [attaque
contre des ressources centralisées](#central). L'abonnement automatique
à une liste donne au fournisseur de la liste le pouvoir de fermer
complètement le réseau I2P.
