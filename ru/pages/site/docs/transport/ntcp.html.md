...
## [Спецификация адресов маршрутизатора]{#ra}
...
sk, prev) 
 ...
 Легенда:
 X, Y: 256 байт публичного ключа DH 
 H(): 32 byte SHA256 Hash
 E(data, session key, IV): AES256 Encrypt
 S(): Signature
 tsA, tsB: временные метки (4 байта, секунды с начала эпохи)
...
1. Алиса генерирует секретное целое число x. Она высчитывает его как
 `X = g^x mod p`.
2. Алиса посылает X Бобу **(Сообщение 1)**.
3. Боб генерирует секретное целое число y. Он высчитывает его как
 `Y = g^y mod p`.
4. Боб посылает Y Алисе.**(Сообщение 2)**
...
(32 bytes) tsB :: 4-байтовая временная метка (секунд с начала эпохи)
padding :: 12 байт случайных данных 
...
#### Сообщение 3 (Подтверждение сессии А)
...
E(sz+Alice.identity+tsA+padding+S(X+Y+Bob.identHash+tsA+tsB), sk,
hX_xor_Bob.identHash\[16:31\])\-\--\> Размер: 448 bytes (typ. for 387
...
Alice\'s 387+ byte \`RouterIdentity\` tsA :: 4-байтовая временная метка
(секунд с начала эпохи) padding :: 0-15 байт случайных данных signature
:: the \`Signature\` of the following concatenated data: X, Y, Bob\'s
...
#### Сообщение 4 (Подтверждение сессии Б)

Это - подпись критических данных. Боб отправляет Алисе:

 \*
\<\-\-\-\-\-\-\-\-\-\-\-\-\-\-\-\-\-\-\-\-\--E(S(X+Y+Alice.identHash+tsA+tsB)+padding,
sk, prev) Размер: 48 bytes (typ. for DSA signature), see notes below 
...
\`RouterIdentity\` padding :: 0-15 байт случайных данных 
...
- Алиса проверяет подпись и при неудаче отбрасывает соединение.
...
Проверка соединений полностью отключена с выпуска 0.9.12.
