...
между маршрутизатором и любым клиентом, который хочет общаться по сети.
Он обеспечивает безопасный и асинхронный обмен сообщениями путем
отправки и получения сообщений через один сокет TCP. При использовании
I2CP клиентское приложение сообщает маршрутизатору, кто он (его
\"адресат\"), какие компромиссы по анонимности, надежности и задержке
необходимо сделать и куда отправлять сообщения. В свою очередь,
маршрутизатор использует I2CP, чтобы сообщить клиенту о прибытии
сообщений и запросить разрешение на использование некоторых туннелей.
...
Порт источника I2P (Gzip mtime)

6-7

Порт назначения I2P (Gzip mtime)
