 Сетевая база
данных 2025-03 0.9.65 

## Обзор

I2P\'s netDb - это специализированная распределенная база данных,
содержащая всего два типа данных - контактную информацию маршрутизатора
(**RouterInfos**) и контактную информацию пункта назначения
(**LeaseSets**). Каждая часть данных подписывается соответствующей
стороной и проверяется каждым, кто их использует или хранит. Кроме того,
данные содержат информацию о \"живости\", что позволяет отбрасывать
неактуальные записи, заменять старые записи новыми и защищать их от
определенных видов атак.

NetDb распределяется с помощью простой техники, называемой
\"floodfill\", когда подмножество всех маршрутизаторов, называемых
\"floodfill routers\", поддерживает распределенную базу данных.

## RouterInfo {#routerInfo}

Когда I2P-маршрутизатор хочет связаться с другим маршрутизатором, ему
необходимо знать некоторые ключевые части данных - все они собираются и
подписываются маршрутизатором в структуру под названием \"RouterInfo\",
которая распространяется с SHA256 идентификатором маршрутизатора в
качестве ключа. Сама структура содержит:

- Идентификационные данные маршрутизатора (ключ шифрования, ключ
 подписи и сертификат)
- Контактные адреса, по которым можно связаться
- Когда это было опубликовано
- Набор произвольных текстовых опций
- Подпись, сгенерированная ключом подписания идентификатора.
...
**caps** (Флаги возможностей - используются для указания участия в
заливке, приблизительной пропускной способности и предполагаемой
достижимости)
...
### Неопубликованные LeaseSets {#unpublished}
...
### Зашифрованные LeaseSet (LS1) {#encrypted}

Этот раздел описывает старый, небезопасный метод шифрования LeaseSets,
используя закрепленный симметричный ключ. См. ниже версию LS2
зашифрованных LeaseSets.
...
### Зашифрованные LeaseSet (LS2) {#encrypted2}
...
### Выбор ключа шифрования (LS2) {#ls2keys}
...
### Роли Floodfill маршрутизаторов
...
Из-за относительно небольшого размера сети и избыточности флудинга,
поиск обычно составляет O(1), а не O(log n). Высока вероятность того,
что маршрутизатор знает маршрутизатор-флудер достаточно близко к ключу,
чтобы получить ответ с первой попытки. В релизах до 0.8.9 маршрутизаторы
использовали избыточность поиска, равную двум (то есть два поиска
выполнялись параллельно к разным пирам), и ни *рекурсивная*, ни
*итеративная* маршрутизация для поиска не были реализованы. Запросы
отправлялись по *многочисленным маршрутам одновременно*, чтобы *снизить
вероятность сбоя запроса*.

Начиная с версии 0.8.9, реализован *iterative lookups* без избыточности
поиска. Это более эффективный и надежный поиск, который будет работать
гораздо лучше, когда не все флудфиллеры известны, и устраняет серьезное
ограничение для роста сети. По мере роста сети, когда каждый
маршрутизатор знает только небольшое подмножество флудфилл-пиров, поиск
станет O(log n). Даже если пир не возвращает ссылки, близкие к ключу,
поиск продолжается со следующим ближайшим пиром, что обеспечивает
дополнительную надежность и предотвращает \"черную дыру\" в пространстве
ключей. Поиск продолжается до тех пор, пока не будет достигнут общий
тайм-аут поиска или пока не будет запрошено максимальное количество
пиров.
...
### Выбор ретрансляторов на основе DHT
