...
kriticky důležité. Bohužel kvůli potřebě spoluúčasti několika peerů při
vytvoření a testování tunelu, je složité rozpoznat příčinu upuštěného
požadavku na vytvoření nebo příčinu selhání testu. Router přiřadí
pravděpodobnost selhání každému z peerů a použije pravděpodobnost při
počítání kapacity. Upuštění požadavků a selhání testů mají výrazně vyšší
váhu než odmítnutí.
