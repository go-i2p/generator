...
+-----------------+-----------------+-----------------+-----------------+
| Role | Preprocessing | Encryption | Postprocessing |
| | | Operation | |
+=================+=================+=================+=================+
...
| | | operations) | |
+-----------------+-----------------+-----------------+-----------------+
| Účastník |   | Decrypt (using | Forward to next |
| | | an encryption | hop |
| | | operation) | |
+-----------------+-----------------+-----------------+-----------------+
| Odchozí koncový |   | Decrypt (using | Reassemble |
| bod | | an encryption | Fragments, |
...
| ------------- | | | |
+-----------------+-----------------+-----------------+-----------------+
| Příchozí brána | Fragment, | Zašifrovat | Forward to next |
| | Batch, and Pad | | hop |
+-----------------+-----------------+-----------------+-----------------+
| Účastník |   | Zašifrovat | Forward to next |
| | | | hop |
+-----------------+-----------------+-----------------+-----------------+
...
Přestože tunely v rámci I2P připomínají síť se spínanými obvody, je vše
v rámci I2P striktně založeno na zprávách - tunely jen účtují triky, aby
se pomohlo organizaci doručení zpráv. Nejsou zde žádné předpoklady
ohledně spolehlivosti nebo pořadí zpráv a opakované přenosy jsou
ponechány v platnosti pro vyšší úrovně (např. knihovna streamování na
klientské vrstvě I2P). To umožňuje I2P využívat techniky omezování,
které jsou k dispozici jak v sítích s přepínáním paketů, tak i v sítích
se spínanými obvody. Každý router může například sledovat klouzavý
průměr množství dat využitých každým z tunelů, kombinovat to se všemi
průměry využití ostatních tunelů ve kterých se router spoluúčastní a
následně může router přijmout nebo odmítnout další požadavky o
spoluúčast na tunelu, v závislosti na tom, jakou má kapacitu a využití.
Na druhou stranu každý router může snadno upustit zprávy, které jsou nad
jeho kapacitu, s využitím výzkumu používaného na běžném internetu.
