...
Náhodné množství pro přidání nebo odebrání délce tunelů. Kladné číslo x
znamená přidání náhodného množství od 0 do x (včetně). Záporné číslo -x
znamená přidání náhodného množství od -x do x (včetně). Router bude
omezovat celkovou délku tunelu na 0 až 7 (včetně). Výchozí rozptyl měl
před vydáním 0.7.6 hodnotu 1.
