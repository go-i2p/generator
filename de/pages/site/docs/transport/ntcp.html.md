 NTCP
(NIO-basiertes TCP) 2021-10 0.9.52 
...
## NTCP-Protokollspezifikation

### Standardnachrichtenformat
...
sk, prev) 
 ...
 Legende:
 X, Y: 256 Byte DH öffentliche Schlüssel
 H(): 32 byte SHA256 Hash
 E(data, session key, IV): AES256 Encrypt
 S(): Signature
 tsA, tsB: timestamps (4 bytes, seconds since epoch)
 sk: 32-Byte-Sitzungsschlüssel
...
1. Alice erzeugt eine geheime Ganzzahl x. Anschließend berechnet sie
 `X = g^x mod p`.
2. Alice sendet X an Bob **(Nachricht 1)**.
3. Bob erzeugt eine geheime Ganzzahl y. Anschließend berechnet er
 `Y = g^y mod p`.
4. Bob sendet Y an Alice.**(Nachricht 2)**
5. Alice kann jetzt `Sitzungsschlüssel = Y^x mod p` berechnen.
6. Bob kann jetzt `Sitzungsschlüssel = X^y mod p` berechnen.
7. Sowohl Alice und Bob haben jetzt einen gemeinsamen Schlüssel
 `Sitzungsschlüssel = g^(x*y) mod p`.
...
\| + + \| HXxorHI \| + + \| \| + + \| \|
+\-\-\--+\-\-\--+\-\-\--+\-\-\--+\-\-\--+\-\-\--+\-\-\--+\-\-\--+ X ::
256 Byte X von Diffie Hellman HXxorHI :: SHA256 Hash(X) xored with
...
\| padding \| +\-\-\--+\-\-\--+\-\-\--+\-\-\--+ + \| \|
+\-\-\--+\-\-\--+\-\-\--+\-\-\--+\-\-\--+\-\-\--+\-\-\--+\-\-\--+ Y ::
256 Byte Y von Diffie Hellman HXY :: SHA256 Hash(X concatenated with Y)
(32 bytes) tsB :: 4 byte timestamp (seconds since the epoch) padding ::
12 Byte zufällige Daten 
...
+\-\-\--+\-\-\--+\-\-\--+\-\-\--+\-\-\--+\-\-\--+\-\-\--+\-\-\--+ 
 ...
 Y: 256 Byte Y von Diffie Hellman
...
E(sz+Alice.identity+tsA+padding+S(X+Y+Bob.identHash+tsA+tsB), sk,
hX_xor_Bob.identHash\[16:31\])\-\--\> Größe: 448 bytes (typ. for 387
...
since the epoch) padding :: 0-15 Byte zufällige Daten signature :: the
...
#### Nachricht 4 (Sitzung bestätigen B)

Dies ist eine Signatur der kritischen Daten. Bob sendet Alice:

 \*
\<\-\-\-\-\-\-\-\-\-\-\-\-\-\-\-\-\-\-\-\-\--E(S(X+Y+Alice.identHash+tsA+tsB)+padding,
sk, prev) Größe: 48 bytes (typ. for DSA signature), see notes below 
...
\`RouterIdentity\` padding :: 0-15 Byte zufällige Daten 
...
- 32 Byte uninterpretierter, ignorierter Daten
- 1-Byte-Größe
